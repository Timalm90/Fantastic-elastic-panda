...
function buildInitialBlendshapes(): BlendshapeValues {
  const initial = {} as BlendshapeValues
  for (const zone of CONTROL_ZONES) {
    for (const key of [zone.x?.positive, zone.x?.negative, zone.y?.positive, zone.y?.negative]) {
      if (key) initial[key as BlendshapeKey] = 0
    }
  }
  return initial
}
...
export const FaceControls: React.FC<FaceControlsProps> = ({ onBlendshapesChange }) => {
  const [blendshapes, setBlendshapes] = useState<BlendshapeValues>(buildInitialBlendshapes)
  const [wrapperSize, setWrapperSize] = useState({ width: 300, height: 500 })

  const wrapperRef = useRef<HTMLDivElement>(null)
  const { beginDrag, applyDrag, endDrag } = useBlendshapeControl(setBlendshapes)
    ...
    <div ref={wrapperRef} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {CONTROL_ZONES.map((zone: ControlZone) => (
        <DragZone
          ...
          onDrag={applyDrag}
          onRelease={endDrag}
          style={{ ...getZoneStyle(zone), pointerEvents: 'auto' }}
        />
      ))}
    </div>
  )
